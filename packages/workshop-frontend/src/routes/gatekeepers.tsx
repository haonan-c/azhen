  ...
  const trailing =
    state === 'expired' && onReconnect ? (
      ...
      >
        ...
        {reconnectBusy ? messages.gatekeepers_opening() : messages.gatekeepers_reconnect()}
      </button>
    ) : (
    ...
    <div className="relative isolate hidden h-[176px] w-[444px] lg:block">
      ...
      <div className="group absolute left-[176px] top-[58px] z-20">
        <button
          ...
          aria-label={messages.gatekeepers_hero_access_label()}
        >
...
function ConnectorsPage() {
  useDocumentTitle(messages.gatekeepers_document_title())
  ...
  const handleConfirmConnect = async (resourceUrlPatterns?: string[]) => {
    ...
    try {
      ...
      toasts.add({ title: messages.gatekeepers_start_connection_failed(), variant: 'error' })
    } finally {
      setConnecting(false)
    }
  }
  ...
  const handleEnsureResources = async (resourceUrlPatterns: string[]) => {
    ...
    try {
      ...
      toasts.add({ title: messages.gatekeepers_request_access_failed(), variant: 'error' })
    } finally {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toasts.add({ title: messages.gatekeepers_disconnect_failed(), variant: 'error' })
    } finally {
  ...
  const handleReconnect = async (accountId: number) => {
    ...
    try {
      ...
      toasts.add({ title: messages.gatekeepers_reconnect_failed(), variant: 'error' })
    } finally {
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-8 sm:py-14">
        ...
        <div className="mb-6 flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={messages.gatekeepers_search()}
        ...
        {loadError && (
          <div className="rounded-2xl border border-kumo-line bg-kumo-base px-4 py-6 text-center">
            <p className="m-0 text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
              {messages.gatekeepers_load_error_title()}
            </p>
            <p className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {messages.gatekeepers_load_error_description()}
            </p>
          </div>
        )}
        ...
        {initialLoading && (
          <div className="rounded-2xl border border-kumo-line bg-kumo-base px-4 py-8 text-center text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
            {messages.gatekeepers_loading()}
          </div>
        )}

        {filteredAccounts.length > 0 && (
          <section className="mb-10">
            <SectionEyebrow label={messages.gatekeepers_connected()} count={filteredAccounts.length} />
                    ...
                    metaLine={
                      ...
                      >
                        {account.credentialsValid
                          ? displayName
                          : messages.gatekeepers_credentials_expired()}
                      </span>
                    }
                    tagline={tagline}
        ...
        {filteredAvailable.length > 0 && (
          <section className="mb-10">
            <SectionEyebrow label={messages.gatekeepers_available()} />
            <div className={sectionGridClass}>
        ...
        {!initialLoading &&
          ...
          filteredAvailable.length === 0 && (
            <EmptyState
              title={
                search
                  ? messages.gatekeepers_no_match_title()
                  : messages.gatekeepers_empty_title()
              }
              description={
                search
                  ? messages.gatekeepers_no_match_description()
                  : messages.gatekeepers_empty_description()
              }
              icon={Plugs}
            />
          )}
