...
import { formatNoun, formatOf, formatPlural } from '../components/format/formats'
      ...
      <DropdownMenu>
        <DropdownMenu.Trigger
          render={
            <button
              type="button"
              aria-label={messages.outputs_actions()}
        ...
        <DropdownMenu.Content className={MENU_CONTENT}>
          <DropdownMenu.Item onClick={onOpen} className={MENU_ITEM}>
            <ArrowSquareOut size={13} className="mr-2" /> {messages.outputs_open()}
          </DropdownMenu.Item>
          <DropdownMenu.Item onClick={onOpenWorkspace} className={MENU_ITEM}>
            <Cube size={13} className="mr-2" /> {messages.outputs_open_workspace()}
          </DropdownMenu.Item>
          {onRename && (
            <DropdownMenu.Item onClick={onRename} className={MENU_ITEM}>
              <PencilSimple size={13} className="mr-2" /> {messages.outputs_rename()}
            </DropdownMenu.Item>
          )}
          {onRemove && (
            <DropdownMenu.Item onClick={onRemove} className={`${MENU_ITEM} text-kumo-danger`}>
              <Trash size={13} className="mr-2" /> {messages.outputs_remove()}
            </DropdownMenu.Item>
          )}
...
function subtitle(output: OutputSummary): string {
  const parts = [output.workspaceTitle || messages.shell_workspace_untitled()]
  if (output.owner) parts.push(messages.outputs_shared_by({ name: output.owner.name }))
  parts.push(messages.outputs_workspace_active({ time: formatRelativeTime(output.lastActive) }))
  return parts.join(' · ')
}
...
function OutputProvenance({ owner }: { owner?: OutputSummary['owner'] }) {
  return (
    <span
      ...
      title={owner
        ? messages.outputs_in_shared_workspace({ name: owner.name })
        : messages.outputs_in_your_workspace()}
    >
      {owner ? <ShareNetwork size={11} /> : <User size={11} />}
      <span className="truncate">
        {owner
          ? messages.outputs_shared_by({ name: owner.name })
          : messages.outputs_created_by_you()}
      </span>
    </span>
  )
}
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-medium leading-[18px] tracking-[-0.25px] text-kumo-default">
            {output.title || messages.outputs_untitled()}
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
          {formatNoun(output.output)} · {output.workspaceTitle || messages.shell_workspace_untitled()}
        </p>
      </div>
      ...
      <div className="hidden shrink-0 items-center gap-6 text-xs text-kumo-inactive lg:flex">
        ...
        <span className="flex w-40 items-center justify-end gap-1 whitespace-nowrap">
          <Clock size={10} />
          {messages.outputs_workspace_active({ time: formatRelativeTime(output.lastActive) })}
        </span>
      </div>
      <OutputMenu onOpen={onOpen} onOpenWorkspace={onOpenWorkspace}
                  onRename={onRename} onRemove={onRemove} />
...
function ScopeSelect({
  ...
  const options: { value: OwnerFilter; label: string }[] = [
    { value: 'all', label: messages.outputs_scope_all() },
    { value: 'mine', label: messages.outputs_scope_mine() },
    { value: 'shared', label: messages.outputs_scope_shared() },
  ]
        ...
        <form onSubmit={(event) => { event.preventDefault(); onSave() }}>
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 text-kumo-subtle">
                {messages.outputs_rename_description({
                  workspaceTitle: output?.workspaceTitle || messages.shell_workspace_untitled(),
                })}
              </Dialog.Description>
            </div>
            <WorkshopIconButton type="button" className="!h-7 !w-7" disabled={busy} aria-label={messages.common_close()} onClick={onClose}>
              <X size={16} />
            </WorkshopIconButton>
          </div>
          <div className="px-5 py-4">
            <label className="block text-[12px] font-medium text-kumo-subtle" htmlFor="rename-output-title">
              {messages.outputs_rename_name()}
            </label>
            <input
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line px-5 py-3">
            ...
            <WorkshopButton tone="primary" type="submit" disabled={busy || !value.trim()}>
              {busy ? messages.outputs_saving() : messages.outputs_save()}
            </WorkshopButton>
          </div>
        </form>
      </Dialog>
...
function OutputsPage() {
  useDocumentTitle(messages.shell_outputs())
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (loadedOnce.current) {
        toastsRef.current.add({ title: messages.outputs_refresh_failed(), variant: 'error' })
      } else {
        setLoadError(true)
      }
    })
  ...
  const saveRename = async () => {
    ...
    try {
      ...
      toasts.add({ title: messages.outputs_rename_failed(), variant: 'error' })
    } finally {
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      toasts.add({ title: messages.outputs_remove_failed(), variant: 'error' })
    } finally {
      gadget?.[Symbol.dispose]()
      overseer?.[Symbol.dispose]()
      setMutationBusy(false)
  ...
  const presentTypes = useMemo(() => {
    ...
    let byId = new Map<string, string>([[generic.id, formatPlural()]])
    for (let offer of formats) {
      if (!byId.has(offer.output.id)) byId.set(offer.output.id, formatPlural(offer.output))
    }
    for (let output of outputs) {
      let format = formatOf(output.output)
      if (!byId.has(format.id)) byId.set(format.id, formatPlural(output.output))
    }
    return [...byId]
  }, [formats, outputs])
    ...
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col px-6 sm:px-10">
      ...
      <div className={`flex flex-col gap-3 px-3 pb-3 sm:flex-row sm:items-center sm:justify-between ${
        ...
        <div className="-mx-1 flex items-center gap-1 overflow-x-auto px-1 sidebar-scroll">
          {showTypeFilters && (
            <>
              <FilterChip active={typeFilter === 'all'} label={messages.outputs_all()} count={inTypeScope.length}
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="relative sm:w-56">
            ...
            <input
              ...
              placeholder={messages.outputs_search()}
              aria-label={messages.outputs_search()}
      ...
      <div className="chat-panel min-h-0 flex-1 overflow-y-auto pb-8 pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">{messages.outputs_load_error()}</p>
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-20 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">
                {isFiltered ? messages.outputs_no_match() : messages.outputs_none()}
              </p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {isFiltered
                  ? messages.outputs_no_match_description()
                  : messages.outputs_none_description()}
              </p>
            </div>
            {/* Offer the deployment's formats here rather than sending them to the home page. */}
            {!isFiltered && <NewFormatRow label={messages.output_format_start_with()} />}
          </div>
        ) : view === 'grid' ? (
          <div className="grid grid-cols-2 gap-4 px-3 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((output) => (
      ...
      <DeleteConfirmationDialog
        ...
        title={messages.outputs_remove_named({
          title: removeOutput?.title || messages.outputs_untitled(),
        })}
        description={
          removeOutput?.owner
            ? messages.outputs_remove_description_shared({
                workspaceTitle: removeOutput.workspaceTitle || messages.shell_workspace_untitled(),
              })
            : messages.outputs_remove_description({
                workspaceTitle: removeOutput?.workspaceTitle || messages.shell_workspace_untitled(),
              })
        }
        confirmLabel={messages.outputs_remove()}
        confirmingLabel={messages.outputs_removing()}
