...
export default function FileSidebar({
  ...
  const handleCreateFile = () => {
    if (!newFileName.trim()) {
      toasts.add({ title: messages.workspace_files_filename_empty(), variant: 'error' })
      return
    }

    if (files.includes(newFileName.trim())) {
      toasts.add({ title: messages.workspace_files_exists(), variant: 'error' })
      return
    }
  ...
  const startDelete = (filename: string) => {
    if (files.length <= 1) {
      toasts.add({ title: messages.workspace_files_cannot_delete_last(), variant: 'error' })
      return
    }
    ...
    <div className="flex h-full w-[244px] flex-col border-r border-kumo-line bg-kumo-base">
      <div className="flex h-9 shrink-0 items-center justify-between gap-2 px-3 pt-3 pb-2">
        <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-kumo-inactive">
          {messages.workspace_files_heading()}
        </span>
        <WorkshopIconButton
          onClick={() => setIsCreateModalOpen(true)}
          disabled={editLocked}
          aria-label={messages.workspace_files_new()}
          title={messages.workspace_files_new()}
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                {messages.workspace_files_new_description()}
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
          ...
          <div className="px-5 py-4">
            <WorkshopInput
              ...
              aria-label={messages.workspace_files_filename()}
              value={newFileName}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line bg-kumo-base px-5 py-3">
            ...
            >
              {messages.workspace_files_create()}
            </WorkshopButton>
          </div>
        </Dialog>
      </Dialog.Root>
      ...
      <DeleteConfirmationDialog
        ...
        title={messages.workspace_files_delete_title()}
        description={messages.workspace_files_delete_description({ filename: deletingFile ?? '' })}
    ...
    >
      ...
      {isRenaming ? (
        <input
          ...
          autoCapitalize="off"
          autoCorrect="off"
          aria-label={messages.workspace_files_rename({ filename })}
        ...
        >
          ...
          {isStreamingActive && (
            <span
              ...
              aria-label={messages.workspace_files_being_edited({ filename })}
              title={messages.workspace_files_agent_editing()}
            />
          )}
        </button>
      )}
      ...
      {!isRenaming && !isDeleted && (
        <DropdownMenu>
          <DropdownMenu.Trigger
            render={(
              <WorkshopIconButton
                aria-label={messages.workspace_files_actions({ filename })}
                onClick={(event) => event.stopPropagation()}
          ...
          >
            ...
            >
              {messages.conversation_attachment_download()}
            </DropdownMenu.Item>
            {!editLocked && (
              <>
                <DropdownMenu.Item
