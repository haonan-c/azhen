...
export type ComposerDraftFormat = {
  start: number;
  length: number;
  /** Canonical catalog noun sent to the agent. */
  noun: string;
  /** Localized noun shown in the composer when it differs from the catalog noun. */
  displayNoun?: string;
  icon: OutputIcon;
};

type StoredComposerDraftFormat = MessageFormatRef & {
  displayNoun?: string;
};
...
export type StoredComposerDraft = {
  version: 1;
  text: string;
  formats: StoredComposerDraftFormat[];
  ...
  for (const token of tokens) {
    ...
    if (token.kind === "format") {
      storedFormats.push({
        ...
        noun: token.noun,
        icon: token.icon,
        ...(token.displayNoun && token.displayNoun !== token.noun
          ? { displayNoun: token.displayNoun }
          : {}),
      });
...
export function decorateComposerDraft(
  ...
  for (const [index, format] of draft.formats.entries()) {
    ...
    const displayNoun = format.displayNoun ?? format.noun;
    const start = text.length;
    text += prefix + displayNoun;
    formats.push({
      start,
      length: prefix.length + format.length,
      noun: format.noun,
      icon: format.icon,
      ...(format.displayNoun ? { displayNoun: format.displayNoun } : {}),
      ...(logo ? { logo } : {}),
    });
...
export function readComposerDraft(key: string | undefined): StoredComposerDraft | undefined {
  ...
  try {
    ...
    const formats: StoredComposerDraftFormat[] = [];
    let previousEnd = 0;
    for (const candidate of record.formats) {
      ...
      if (!Number.isInteger(format.position) || !Number.isInteger(format.length) ||
          ...
          format.displayNoun !== undefined && typeof format.displayNoun !== "string" ||
          !isOutputIcon(format.icon)) {
        return undefined;
      }
      const position = format.position as number;
      const length = format.length as number;
      const displayNoun = (format.displayNoun ?? format.noun) as string;
      if (position < previousEnd || format.noun.length === 0 || displayNoun.length === 0 ||
          length !== displayNoun.length ||
          record.text.slice(position, position + length) !== displayNoun) {
        return undefined;
      }
      formats.push({
        position,
        ...
        noun: format.noun,
        icon: format.icon,
        ...(format.displayNoun !== undefined ? { displayNoun } : {}),
      });
