...
> **本部署说明：此技能在本部署不可用。** 它唯一的实现路径是运行时内置的生成式图像模型（Codex `image_gen`），没有任何 API 兜底；本部署没有这类模型。遇到"做小红书信息图/组图"类请求，直接告知用户"本部署当前不支持 AI 生成图片"；需要可编辑、文字精确的图文，改用 `xhs-html`（HTML 排版 + `env[N].renderImage()` 导出 PNG）。以下原文保留仅供参考。
