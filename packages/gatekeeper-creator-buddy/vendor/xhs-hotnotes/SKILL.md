...
description: 通过 Creator Buddy 的只读 Guaikei 会话搜索公开的小红书笔记，帮助创作者发现趋势和选题。
---

# 小红书笔记搜索

通过本次对话中的 Creator Buddy 绑定搜索公开的小红书笔记。所有数据查询都使用会话方法。
不要运行本地命令，不要请求用户提供 API Key，也不要直接访问第三方接口。

## 能力边界

- 支持按一个关键词搜索，也支持用空关键词查询全站结果。
- 多关键词查询时，每个关键词单独调用一次，再按笔记 `id` 去重。
- `limit` 控制单次返回数量；默认值是 20。
- 结果只包含 Guaikei 实际返回的字段。不要推算或编造评分、排名、时间范围、粉丝数或互动数。
...
从用户请求中提取具体关键词，例如“职场穿搭”或“减脂餐”。

- 用户没有指定赛道，只要求查看热门内容时，使用空字符串。
- 用户给出多个关键词时，分别查询。
- 用户给出很宽的分类词时，可以先建议几个细分词并请求用户选择。不要声称这些建议来自接口。

### 2. 调用 Creator Buddy

在执行代码时，先从当前环境中确定 Creator Buddy 的绑定编号，再调用：

```javascript
const notes = await env[N].searchXiaohongshuNotes(keyword, { limit: 20 });
```

`env[N]` 是本次对话中的 Creator Buddy 绑定。不要把 `N` 当成固定值。
...
- 笔记标题、互动量、发布时间等供应商字段可能位于 `note.extra`。
- 头像等未标准化的作者字段可能位于 `note.user.extra`。
...
- 鉴权或服务错误：直接说明 Creator Buddy 查询失败。不要要求用户配置部署凭证。
- 用户要求日期筛选、订阅、报告文件或服务端分页时，说明本部署不支持，并提供已支持的关键词查询方式。

## 输出前检查

- 查询是否只通过 Creator Buddy 会话完成？
- 所有数据是否来自本次返回值？
