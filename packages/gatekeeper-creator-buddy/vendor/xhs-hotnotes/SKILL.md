...
description: 通过 Creator Buddy 的只读 TikHub 会话搜索公开的小红书笔记，帮助创作者发现趋势和选题。
---
...
在执行代码时，先从当前环境中确定 Creator Buddy 的绑定编号。搜索、字段映射和返回必须在**同一次代码执行**中完成。不同代码执行不保留局部变量；拆成两次会产生重复的付费查询。
