...
description: 【公众号配图版】把公众号文章/段落拆成逻辑关系图，做成可直接放进公众号的配图。生成逻辑关系 SVG，默认输出独立 HTML，需要 PNG 时经本部署的渲染能力导出。支持递进、流程、循环、层次、对比、矩阵六种关系。当用户给文章/段落/论述并要求生成逻辑图、关系图、配图、可视化时触发。支持中文，一次可拆多张。
license: MIT
metadata:
  version: "1.1"
...
**出图方式（本部署）**：生成自包含 HTML（内含 SVG）；要 PNG 时，把该 HTML 传给 `env[N].renderImage(html, { width, height })`（`env[N]` 指本次对话里的 Creator Buddy 绑定，尺寸按下方"公众号常用尺寸"选）。原版提到的"交给 Codex/workbuddy 内置出图模型出手绘感图片"这条路径本部署不可用——本部署没有生成式图像模型，只能渲染你写出的 HTML/SVG，做不出手绘/插画感的图。
