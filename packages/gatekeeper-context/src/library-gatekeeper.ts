...
export class ContextAccount
  ...
  async startAppUi(context: AppUiContext): Promise<GatekeeperUiFrame> {
    ...
    let billingAuthorizer = context.billingAuthorizer.dup();
    let ui: RpcStub<ContextApiImpl>;
    try {
      ui = new RpcStub(new ContextApiImpl(
        this.env, this.ctx.props.sharingDomain, this.ctx.props.accountId, context.isAdmin,
        this.#collections(), this.#userLibraries(), this.#registries(), billingAuthorizer));
    } catch (err) {
...
export class ContextGatekeeper
  ...
  #newReadSession(
      authorizer: NativeRpcStub<ObservationAuthorizer>,
      readBillingMethodKey?: string): LibraryReadSession {
  ...
  async #invokeAgentSkillCommand(
      ...
      Promise<SlashCommandResult> {
    using session = this.#newReadSession(
      authorizer,
