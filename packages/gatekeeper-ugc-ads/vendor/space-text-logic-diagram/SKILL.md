...
**出图方式（本部署）**：生成自包含 HTML（内含 SVG）；要 PNG 时，把该 HTML 传给 `env[N].renderImage(html, { width, height })`（`env[N]` 指本次对话里的 UGC Ads 绑定，尺寸按下方"公众号常用尺寸"选）。原版提到的"交给 Codex/workbuddy 内置出图模型出手绘感图片"这条路径本部署不可用——本部署没有生成式图像模型，只能渲染你写出的 HTML/SVG，做不出手绘/插画感的图。
