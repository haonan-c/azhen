...
1. **安全探查**：只读列名 + dtypes + 前 5 行。本部署没有可调用的 `scripts/xhs_notes.py`，用你自己的代码执行能力写等价的 `nrows=5` 探查代码（见下）。
...
本部署没有可调用的 `scripts/xhs_notes.py`（原版是本地 Python 脚本）；用你自己的代码执行能力写等价逻辑，按同样的三步走：

1. **probe**：只读列名、数据类型、前 5 行（先做这步，别的都在这之后）
2. **metrics**：做列名映射（常见别名如"曝光量/展现量/浏览量"混用）+ 单位自适应（识别 `5.2` / `0.052` / `"5.2%"` 三种写法）+ 算全部比率 + 质量体检
3. **group**：按指定列分组，算中位数对比、分位数、异常值

列名对不上、单位看不准时，**不要猜，去问用户**——这条和原版一致。
