...
搜索和输出必须在**同一次代码执行**中完成。代码执行器只展示 `console.log` 的内容；不要用函数 `return` 输出搜索结果。下面是完整调用形状，执行前把关键词和 `env[N]` 换成用户原话中的关键词及当前对话实际提供的 UGC Ads 绑定：

```javascript
export default async function(self, env, ctx) {
  const notes = await env[N].searchXiaohongshuNotes("用户原话中的关键词", {
    sort: 0,
    time: 2,
    limit: 20,
  });
...
一次成功调用已经返回完整的选题分析字段。不要为检查字段再查一次。如果执行后没有可见输出，先检查代码里是否有上面的 `console.log`，不要重新发起付费查询。
