...
This is a fork (`haonan-c/azhen`, remote `origin`) tracking `cloudflare/cloudflare-os` (remote `upstream`) for active downstream development while upstream keeps moving:
* `main` is a pure mirror of `upstream/main` — never commit here directly, only fast-forward it (`git fetch upstream && git checkout main && git merge --ff-only upstream/main && git push origin main`).
* `dev` is the default branch (both locally and as the GitHub default branch on `origin`) — all downstream development happens here or on branches cut from it.
* To pull upstream changes into ongoing work: update `main` as above, then `git checkout dev && git rebase main` (resolve conflicts, then `git push origin dev --force-with-lease`).
